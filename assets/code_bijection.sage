...
# ==================================================
# 1. PERMUTATIONS 312-ÉVITANTES
# ==================================================

def avoids_312(pi):
    """
    Vérifie si une permutation évite le motif 312.
    """
    n = len(pi)
    for i in range(n):
...
def Av_312(n):
    """
    Génère toutes les permutations de taille n évitant 312.
    """
    return [list(p) for p in Permutations(n) if avoids_312(p)]


# ==================================================
# 2. FONCTIONS COMBINATOIRES : mu, records, std
# ==================================================

def mu_list(pi):
    """
    Calcule la liste des mu pour une permutation.
    """
    M = 0
    out = []
    for x in pi:
...
def expected_mu(pi):
    """
    Calcule la liste des mu attendue pour une permutation 312-évitable.
    """
    k = pi.index(1)
    L = pi[:k]
    R = pi[k+1:]
...
def records(pi):
    """
    Trouve les records (nouveaux maximums) dans une permutation.
    """
    M = 0
    out = []
    for x in pi:
...
def std(p):
    """
    Standardise une séquence d'entiers distincts.
    """
    if not p:
        return []
    vals = sorted(p)
...
# ==================================================
# 3. BIJECTION PHI : PERMUTATION → DYCK
# ==================================================

def Phi(pi):
    """
    Transforme une permutation en mot de Dyck.
    """
    M = 0
    w = []
    for x in pi:
...
# ==================================================
# 4. ARBRES : gamma (Dyck → arbre), beta_min (perm → arbre)
# ==================================================

def gamma(w):
    """
    Décompose un mot de Dyck en arbre binaire.
    """
    if w == "":
        return ()
...
def beta_min(pi):
    """
    Construction récursive d'arbre en prenant le minimum.
    """
    if len(pi) == 0:
        return ()
...
# ==================================================
# 5. TESTS DU THÉORÈME PRINCIPAL
# ==================================================

def check_theorem(nmax):
    """
    Vérifie que gamma(Phi(pi)) == beta_min(pi) pour tout pi évitant 312.
    """
...
# ==================================================
# 6. TESTS DES PROPRIÉTÉS INTERMÉDIAIRES
# ==================================================

def check_conjecture_intermediaire(pi):
    """
    Vérifie que Phi(pi) = u + Phi(std(L)) + d + Phi(std(R)).
    """
    m = min(pi)
    i = pi.index(m)
    L = pi[:i]
...
def check_L_formula(pi):
    """
    Vérifie que la partie gauche de Phi(pi) = u + Phi(std(L)).
    """
    k = pi.index(1)
    L = pi[:k]
    if not L:
...
def check_WL_formula(pi):
    """
    Vérifie la construction directe de la partie gauche de Phi.
    """
    k = pi.index(1)
    L = pi[:k]
    if not L:
...
def check_first_return_block(w):
    """
    Extrait le premier bloc de retour à 0 d'un mot de Dyck.
    """
    bal = 0
...
def check_mu_structure(nmax):
    """
    Vérifie la structure de mu pour les permutations 312-évitées.
    """
    for n in range(1, nmax+1):
        for pi in Av_312(n):
            mu_calc = mu_list(pi)
...
def run_all_tests(nmax: int = 10, verbose: bool = True):
    ...
    """
    print(f"tests des permutations jusqu'à n = {nmax}\n")
    print("-"*40)

    print("\n1. Test du corollaire")
    count = 0
    for n in range(1, nmax+1):
        for pi in Av_312(n):
            count += 1
    ...
    print(f"Aucun contre exemple (testés : {count})")

    print("\n2. Test de la structure de mu")
    if check_mu_structure(nmax):
        print("structure vérifiée")

    print("\n3. Test de la conjecture intermédiaire")
    succes_count = 0
    total_count = 0
    for n in range(1, nmax+1):
        for pi in Av_312(n):
            ...
            if check_conjecture_intermediaire(pi):
                ...
                print(f"échec pour pi = {pi}")
    print(f"réussite : {succes_count}/{total_count}")
...
# ==================================================
# 8. AUTRES BIJECTIONS (Krattenhaler, Callan, BJS, Bandlow)
# ==================================================
...
def callan(pi):
    ...
    AD = []
    for i in range(n-1):
        AD.append('A' if pi[i] < pi[i+1] else 'D')

    for c in AD:
        w.append('u' if c == 'A' else 'd')
